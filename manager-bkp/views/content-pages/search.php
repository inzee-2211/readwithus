    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            case 'listserial':
                ...
                break;
            case 'cpage_title':
                $td->appendElement('plaintext', [], empty($row[$key]) ? Label::getLabel('LBL_N/A') : $row[$key], true);
                break;
            case 'action':
                if ($canEdit) {
                    ...
                    $innerLiEdit->appendElement('a', ['href' => 'javascript:void(0)', 'class' => 'button small green', 'title' => Label::getLabel('LBL_Edit'), "onclick" => "addFormNew(" . $row['cpage_id'] . ")"], Label::getLabel('LBL_Edit'), true);
                    $innerLiDelete = $innerUl->appendElement("li");
                    $innerLiDelete->appendElement('a', ['href' => "javascript:void(0)", 'class' => 'button small green', 'title' => Label::getLabel('LBL_Delete'), "onclick" => "deleteRecord(" . $row['cpage_id'] . ")"], Label::getLabel('LBL_Delete'), true);
                }
                break;
            default:
