...
class WithdrawRequestsController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch = new SearchBase(WithdrawRequest::DB_TBL, 'withdrawal');
        ...
        if ($post['minprice'] > 0) {
            $srch->addCondition('withdrawal.withdrawal_amount', '>=', $post['minprice']);
        }
        if ($post['withdrawal_id'] > 0) {
            $srch->addCondition('withdrawal.withdrawal_id', '=', $post['withdrawal_id']);
        }
        if ($post['maxprice'] > 0) {
            $srch->addCondition('withdrawal.withdrawal_amount', '<=', $post['maxprice']);
        }
        if ($post['status'] >= 0) {
        ...
        foreach ($records as $key => $record) {
            $record['txnfee'] = json_decode($record['pmethod_fees'], 1);
            $records[$key] = $record;
        }
        $this->sets([
            'arrListing' => $records,
    ...
    private function getSearchForm(): Form
    {
        ...
        $fld->requirements()->setRequired(false);
        $fld->requirements()->setRange(1, 9999999999);
        $frm->addSelectBox(Label::getLabel('LBL_STATUS'), 'status', ['-1' => Label::getLabel('LBL_DOES_NOT_MATTER')] + WithdrawRequest::getStatuses(), '', [], '');
