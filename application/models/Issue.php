...
class Issue extends MyAppModel
{
    ...
    {
        $sessionType = ($data['repiss_record_type'] == AppConstant::LESSON) ? Label::getLabel('LBL_LESSON') : Label::getLabel('LBL_CLASS');
        ...
        $vars = [
            ...
            '{schedule_date}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'Y-m-d', $teacherData['user_timezone']),
            '{start_time}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'H:i:s', $teacherData['user_timezone']),
            '{end_time}' => MyDate::formatDate($data['ordles_lesson_endtime'], 'H:i:s', $teacherData['user_timezone']),
            '{issue_reason}' => $data['repiss_title'],
            '{learner_comment}' => $data['repiss_comment'],
        ];
    ...
    private function sendIssueResolvedMailToLearner(array $data): bool
    {
        ...
        $vars = [
            ...
            '{session_type}' => $sessionType,
            '{class_lesson_name}' => $data['ordles_title'],
            '{schedule_date}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'Y-m-d', $learnerData['user_timezone']),
            '{start_time}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'H:i:s', $learnerData['user_timezone']),
            '{end_time}' => MyDate::formatDate($data['ordles_lesson_endtime'], 'H:i:s', $learnerData['user_timezone']),
            '{issue_resolve_type}' => self::getActionsArr()[$logs['reislo_action']],
            '{teacher_comment}' => $logs['reislo_comment'],
        ];
    ...
    {
        $sessionType = ($data['repiss_record_type'] == AppConstant::LESSON) ? Label::getLabel('LBL_LESSON') : Label::getLabel('LBL_CLASS');
        $learnerData = User::getAttributesById($data['learner_id'], ['user_timezone', 'user_lang_id']);
        ...
        $vars = [
            ...
            '{session_type}' => $sessionType,
            '{class_lesson_name}' => $data['ordles_title'],
            '{schedule_date}' => date("Y-m-d", strtotime($data['ordles_lesson_starttime'])),
            '{start_time}' => date("H:i:s", strtotime($data['ordles_lesson_starttime'])),
            '{end_time}' => date("H:i:s", strtotime($data['ordles_lesson_endtime'])),
            '{time_offset}' => "(" . CONF_SERVER_TIMEZONE . " " . MyDate::getOffset(CONF_SERVER_TIMEZONE) . ")",
            '{learner_comment}' => $logs['reislo_comment'],
        ];
    ...
    private function sendIssueClosedMail(array $data): bool
    {
        ...
        $vars = [
            '{session_type}' => $sessionType,
            '{class_lesson_name}' => $data['ordles_title'],
            '{resolution_action}' => Issue::getActionsArr()[$logs['reislo_action']],
            '{admin_comment}' => $logs['reislo_comment'],
            '{schedule_date}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'Y-m-d', $learnerData['user_timezone']),
            '{start_time}' => MyDate::formatDate($data['ordles_lesson_starttime'], 'H:i:s', $learnerData['user_timezone']),
            '{end_time}' => MyDate::formatDate($data['ordles_lesson_endtime'], 'H:i:s', $learnerData['user_timezone']),
        ...
        $vars['{schedule_date}'] = MyDate::formatDate($data['ordles_lesson_starttime'], 'Y-m-d', $teacherData['user_timezone']);
        $vars['{start_time}'] = MyDate::formatDate($data['ordles_lesson_starttime'], 'H:i:s', $teacherData['user_timezone']);
        $vars['{end_time}'] = MyDate::formatDate($data['ordles_lesson_endtime'], 'H:i:s', $teacherData['user_timezone']);
    ...
    private function executeLessonTransactions(int $refundPercent, int $action, bool $teacherPaid): bool
    {
        ...
        if ($issue['repiss_record_type'] == AppConstant::GCLASS) {
            ...
            $paymentComment = Label::getLabel('LBL_PAYMENT_ON_CLASS_{recordid}');
            $refundComment = Label::getLabel('LBL_{percent}_REFUND_ON_CLASS_{recordid}');
        } else {
            $orderId = $issue['ordles_order_id'];
            $itemAmount = $issue['ordles_amount'];
            $paymentComment = Label::getLabel('LBL_PAYMENT_ON_LESSON_{recordid}');
            $refundComment = Label::getLabel('LBL_{percent}_REFUND_ON_LESSON_{recordid}');
        }
        /* Refund to Student */
        $price = ($issue['ordles_amount'] - $issue['ordles_discount']);
    ...
    {
        $earnings = $issue['ordles_amount'] - ($issue['ordles_discount'] + $issue['ordles_refund'] + $issue['paymentAmount']);
        if ($issue['repiss_record_type'] == AppConstant::LESSON) {
            $lesson = new Lesson($issue['repiss_record_id'], 0, 0);
            $lesson->setFldValue('ordles_updated', date('Y-m-d H:i:s'));
    ...
    private function sendUserNotification(int $langId = 0): bool
    {
        ...
        if (!$notifi->sendNotification(['{link}' => $link, '{lesson-id}' => $lessonId], $userType)) {
    ...
    public function completedLessonSettlement(): bool
    {
        ...
        $srch->joinTable(Issue::DB_TBL, 'LEFT JOIN', 'repiss.repiss_record_id = ordles.ordles_id', 'repiss');
        $srch->addMultipleFields([
            'ordles.ordles_id', 'ordles.ordles_teacher_paid', 'ordles.ordles_order_id',
            'ordles.ordles_teacher_id', 'ordles.ordles_amount', 'ordles.ordles_discount', 'ordles.ordles_commission'
        ]);
        ...
        while ($lesson = FatApp::getDb()->fetch($resultSet)) {
            ...
            $earnings = $lesson['ordles_amount'] - ($lesson['ordles_discount'] + $teacherAmount);
    ...
    public function completedClassSettlement(): bool
    {
        ...
        $srch->joinTable(Issue::DB_TBL, 'LEFT JOIN', 'repiss.repiss_record_id = ordcls.ordcls_id and '
            . ' repiss.repiss_reported_by = orders.order_user_id', 'repiss');
        $srch->addMultipleFields([
            'ordcls.ordcls_order_id', 'ordcls.ordcls_grpcls_id', 'ordcls.ordcls_id', 'ordcls.ordcls_discount',
            'ordcls.ordcls_commission', 'grpcls.grpcls_teacher_id', 'ordcls.ordcls_amount', 'ordcls.ordcls_teacher_paid'
        ]);
        ...
        $resultSet = $srch->getResultSet();
        $db = FatApp::getDb();
        while ($class = $db->fetch($resultSet)) {
            if (!$db->startTransaction()) {
                $this->error = Label::getLabel('LBL_PLEASE_TRY_AGAIN');
                return false;
            }
            ...
            $earnings = $class['ordcls_amount'] - ($class['ordcls_discount'] + $teacherAmount);
            $record->setFldValue('ordcls_earnings', FatUtility::float($earnings));
            if (!$record->update($whr)) {
                $this->error = $record->getError();
                $db->rollbackTransaction();
     ...
     * @return string|array
     */
    public static function getActionsArr(int $key = null)
    {
        $arr = [
            ...
            static::ACTION_ESCALATE_TO_ADMIN => Label::getLabel('LBL_ESCALATE_TO_SUPPORT_TEAM')
        ];
        return AppConstant::returArrValue($arr, $key);
    }
