 ...
 * @author Fatbit Team
 */
class CourseManagement extends YocoachSearch
{
    ...
    public function __construct(int $langId, int $userId, int $userType)
    {
        ...
        $this->table = 'tbl_course_management';
        $this->alias = 'question';
        parent::__construct($langId, $userId, $userType);


        $this->joinTable('course_topics', 'LEFT JOIN', 'course_topics.id = question.topic', 'course_topics');
        $this->joinTable('course_subjects', 'LEFT JOIN', 'course_subjects.id = question.subject', 'course_subjects');
        $this->joinTable('course_levels', 'LEFT JOIN', 'course_levels.id = question.level', 'course_levels');
        $this->joinTable('course_year', 'LEFT JOIN', 'course_year.id = question.year_id', 'course_year');

        $this->joinTable('course_tier', 'LEFT JOIN', 'course_tier.id = question.tier', 'course_tier');

        $this->joinTable('course_type', 'LEFT JOIN', 'course_type.id = question.type', 'course_type');
        $this->joinTable('course_examboards', 'LEFT JOIN', 'course_examboards.id = question.examBoards', 'course_examboards');
    ...
    public function applySearchConditionsadmin(array $post): void
    {

        if (!empty($post['keyword'])) {
            $keyword = trim($post['keyword']);
            $cnd = $this->addCondition('course_levels.level_name', 'LIKE', '%' . $keyword . '%');
        }
        if (!empty($post['subject'])) {
            $subject = trim($post['subject']);
            $cnd = $this->addCondition('course_subjects.subject', 'LIKE', '%' . $subject . '%');
        }

        //  if (!empty($post['year'])) {
        //     $year = trim($post['year']);
        //     $cnd = $this->addCondition('course_year.year', 'LIKE', '%' . $year . '%');
        // }

        if (!empty($post['topic'])) {
            $topic = trim($post['topic']);
            $cnd = $this->addCondition('course_topics.topic', 'LIKE', '%' . $topic . '%');
        }

        if (!empty($post['subtopic'])) {
            $subtopic = trim($post['subtopic']);
            $cnd = $this->addCondition('course_topics.topic', 'LIKE', '%' . $subtopic . '%');
        }
    }
    ...
    public function applySearchConditions(array $post): void
    {
        ...
        if (!empty($post['grpcls_tlang_id'])) {
            $this->addCondition('question.question_type', '=', $post['grpcls_tlang_id']);
        }
    ...
    public static function getSearchForm(int $usertype, $forCalendarView = false): Form
    {
        ...
        $frm->addTextBox(Label::getLabel('LBL_TITLE'), 'title');
        $teachLangs = TeachLanguage::getAllLangs(MyUtility::getSiteLangId(), true);
        $questionTypes = [
            1 => Label::getLabel('LBL_SINGLE_CHOICE'),   // Single Choice Question
            2 => Label::getLabel('LBL_MULTIPLE_CHOICE'), // Multiple Choice Question
            3 => Label::getLabel('LBL_TEXT_BASED')       // Text-based Question
        ];
        ...
        $fld = $frm->addSelectBox(Label::getLabel('LBL_CATEGORY'), 'grpcls_q_cat', $categories, '', [], Label::getLabel('LBL_SELECT'));

        $fld = $frm->addSelectBox(Label::getLabel('LBL_SUBCATEGORY'), 'grpcls_subcate_id', [], '', [], Label::getLabel('LBL_SELECT'));
