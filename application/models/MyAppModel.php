...
class MyAppModel extends FatModel
{
     ...
     * @param int $recordId
     * @param int $activeFld
     * @param type $deletedFld
     * @return array
     */
    public static function getAllNames(bool $assoc = true, int $recordId = 0, int $activeFld = null, int $deletedFld = null)
    {
        ...
        if ($deletedFld != null) {
            $srch->addCondition($deletedFld, '=', AppConstant::NO);
        }
        if ($recordId > 0) {
    ...
    public function assignValues(array $arr, bool $handleDates = false, string $mysqlDateFormat = '', string $mysqlDatetimeFormat = '', bool $executeMysqlFunctions = false)
    {
        // echo '<pre>';
        // print_r($arr);
        // die('Debugging input array to assignValues');
    ...
    public function changeStatus($v = 1): bool
    {
        ...
        $db = FatApp::getDb();
        if (!$db->updateFromArray(static::DB_TBL, [static::DB_TBL_PREFIX . 'active' => $v],
