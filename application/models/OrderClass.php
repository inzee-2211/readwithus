...
class OrderClass extends MyAppModel
{
    ...
    public function refundToLearner(array $classes, float $refundPercent)
    {
        foreach ($classes as $value) {
            $price = $value['ordcls_amount'] - $value['ordcls_discount'];
            ...
            if ($refund > 0) {
                ...
                $comment = Label::getLabel('LBL_{refund}%_CANCEL_REFUND_ON_CLASS_{classid}');
    ...
    public function feedback(array $post): bool
    {
        ...
        $this->assignValues(['ordcls_reviewed' => AppConstant::YES, 'ordcls_updated' => date('Y-m-d H:i:s')]);
        if (!$this->save()) {
            $db->rollbackTransaction();
            return false;
        }
    ...
    public function getClassToStart(int $langId)
    {
        ...
        $srch->addMultipleFields([
            ...
            'learner.user_email as learner_email', 'learner.user_id as learner_id'
        ]);
    ...
    public function getClassToComplete()
    {
        ...
        $srch->addDirectCondition('ordcls_starttime IS NOT NULL');
    ...
    public function getClassToCancel(int $langId = 0)
    {
        ...
        $srch->addMultipleFields([
            'orders.order_id', 'ordcls.ordcls_status', 'ordcls.ordcls_amount', 'ordcls.ordcls_discount',
            ...
            'grpcls_start_datetime', 'ordcls.ordcls_id', 'grpcls_booked_seats'
        ]);
        $row = FatApp::getDb()->fetch($srch->getResultSet());
        if (empty($row)) {
            $this->error = Label::getLabel('LBL_CLASS_NOT_FOUND');
    ...
    public function getClassToFeedback()
    {
        ...
        $srch->addCondition('ordcls_reviewed', '=', AppConstant::NO);
        $srch->addMultipleFields([
            'grpcls.grpcls_id',
