...
class Availability extends FatModel
{
     ...
     */
    public function getAvailability(string $startdate, string $enddate, int $sub = 0): array
    {
        if (AppConstant::YES == $sub) {
        ...
        while ($row = FatApp::getDb()->fetch($resultSet)) {
            if (AppConstant::YES == $sub && $row['avail_endtime'] > $subEndDate) {
