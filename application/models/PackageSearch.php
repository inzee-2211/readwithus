...
class PackageSearch extends YocoachSearch
{
    ...
    public function fetchAndFormat(): array
    {
        ...
        $cancelDuration = FatApp::getConfig('CONF_PACKAGE_CANCEL_DURATION', FatUtility::VAR_INT, 24);
        $currentTimeUnix = strtotime(MyDate::formatDate(date('Y-m-d H:i:s')));
        $teachLangs = TeachLanguage::getNames($this->langId, array_column($rows, 'grpcls_tlang_id'));
