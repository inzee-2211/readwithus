...
class IssueSearch extends YocoachSearch
{
    ...
    public function fetchAndFormat(): array
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if (AppConstant::LESSON == $row['repiss_record_type']) {
                $tlangName = $teachLangs[$row['ordles_tlang_id']] ?? '';
                $row = array_merge($row, [
                ...
                $row = array_merge($row, [
                    ...
                    'ordles_tlang_name' => $teachLangs[$row['grpcls_tlang_id']] ?? '',
    ...
    public static function getSearchForm(): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_CLASS_TYPE'), 'class_type', AppConstant::getClassTypes())->requirements()->setIntPositive();
        $frm->addSelectBox(Label::getLabel('LBL_ISSUE_STATUS'), 'repiss_status', Issue::getStatusArr())->requirements()->setIntPositive();
