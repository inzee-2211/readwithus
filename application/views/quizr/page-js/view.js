$(document).ready(function () {
    showPreviewVideo = function (courseId) {
        fcom.ajax(fcom.makeUrl('Quizr', 'previewVideo', [courseId]), '', function (resp) {
            $.facebox(resp);
        });
    };
    toggleCourseFavorite = function (courseId, el) {
        ...
        fcom.updateWithAjax(fcom.makeUrl('Quizr', 'toggleFavorite', [], confWebDashUrl), data, function (resp) {
            if (status == 0) {
    ...
    reviews = function () {
        ...
        fcom.ajax(fcom.makeUrl('Quizr', 'reviews'), data, function (resp) {
    ...
    openMedia = function (rsrcId) {
        fcom.ajax(fcom.makeUrl('Quizr', 'resource', [rsrcId]), '', function (resp) {
