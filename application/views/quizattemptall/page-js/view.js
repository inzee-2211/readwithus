$(document).ready(function () {
    showPreviewVideo = function (courseId) {
        fcom.ajax(fcom.makeUrl('Quizattempt', 'previewVideo', [courseId]), '', function (resp) {
            $.facebox(resp);
        });
    };
    toggleCourseFavorite = function (courseId, el) {
        ...
        fcom.updateWithAjax(fcom.makeUrl('Quizattempt', 'toggleFavorite', [], confWebDashUrl), data, function (resp) {
            if (status == 0) {
    ...
    reviews = function () {
        ...
        fcom.ajax(fcom.makeUrl('Quizattempt', 'reviews'), data, function (resp) {
    ...
    openMedia = function (rsrcId) {
        fcom.ajax(fcom.makeUrl('Quizattempt', 'resource', [rsrcId]), '', function (resp) {
