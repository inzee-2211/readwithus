$(document).ready(function () {
    showPreviewVideo = function (courseId) {
        fcom.ajax(fcom.makeUrl('Quizresults', 'previewVideo', [courseId]), '', function (resp) {
            $.facebox(resp);
        });
    };
    toggleCourseFavorite = function (courseId, el) {
        ...
        fcom.updateWithAjax(fcom.makeUrl('Quizresults', 'toggleFavorite', [], confWebDashUrl), data, function (resp) {
            if (status == 0) {
    ...
    reviews = function () {
        ...
        fcom.ajax(fcom.makeUrl('Quizresults', 'reviews'), data, function (resp) {
    ...
    openMedia = function (rsrcId) {
        fcom.ajax(fcom.makeUrl('Quizresults', 'resource', [rsrcId]), '', function (resp) {
