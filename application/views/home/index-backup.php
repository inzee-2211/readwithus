...
$(".faq__trigger-js").click(function(e) {
 ...
 <script>
const baseUrl = "<?php echo getBaseUrl(); ?>"; // Your site base URL

const steps = [
  {
    title: "Select Level",
    options: [],
    url: "/api.php?url=getCourses",  // Your levels endpoint
    paramKey: null,
  },
  {
    title: "Select Subject",
    options: [],
    url: "/api.php?url=getSubjects",
    paramKey: "levelId",
  },
    {
    title: "Select Examboard",
    options: [],
    url:  "/api.php?url=getExamboards"
  },
  {
    title: "Select Tier",
    options: [],
    url:  "/api.php?url=getTiers"
  }
];
...
function renderStep(stepIndex) {
  ...
  title.className = 'txt-select-course';
  ...
  if (stepIndex > 0) {
    ...
    backBtn.className = 'back-btn ft-gothic btn btn--bordere';
  ...
  step.options.forEach(opt => {
    ...
    btn.innerHTML = `<span>${displayName}</span><img src="http://readwithus.org.uk/public/assets/img/right-arrow.svg" alt="Arrow icon" class="arrow">`;

    btn.onclick = () => {
      // Save selected value based on step
      if(step.title === "Select Level") {
        selectedValues.levelId = optId;
      } else if(step.title === "Select Year") {
        selectedValues.year = optId;
      ...
      if(currentStep < steps.length) {
        ...
        currentStep = 0;
       // alert(`Selected Level: ${selectedValues.levelId}, Year: ${selectedValues.year}, Subject: ${selectedValues.subjectId}`);
...
openBtn.addEventListener('click', () => {
  if(container.style.display === 'block') {
    ...
    if(steps[0].url && steps[0].options.length === 0) {
      fetchOptionsForStepWithParam(0, selectedValues)
        .then(() => renderStep(0));
    } else {
      renderStep(0);
    }
