...
class Navigation
{
    ...
    {
        $headerNav = FatCache::get('headerNav', CONF_HOME_PAGE_CACHE_TIME, '.txt');
        if (!empty($headerNav)) {
            return unserialize($headerNav);
        }
        $headerNav = self::getNavigation(Navigations::NAVTYPE_HEADER);
        FatCache::set('headerNav', serialize($headerNav), '.txt');
        return $headerNav;
    }
    ...
    {
        $footerOneNav = FatCache::get('footerOneNav', CONF_HOME_PAGE_CACHE_TIME, '.txt');
        if (!empty($footerOneNav)) {
            $footerOneNav = unserialize($footerOneNav);
        } else {
            $footerOneNav = self::getNavigation(Navigations::NAVTYPE_FOOTER_ONE);
            FatCache::set('footerOneNav', serialize($footerOneNav), '.txt');
        }
        return $footerOneNav;
    }
    ...
    {
        $footerTwoNav = FatCache::get('footerTwoNav', CONF_HOME_PAGE_CACHE_TIME, '.txt');
        if (!empty($footerTwoNav)) {
            $footerTwoNav = unserialize($footerTwoNav);
        } else {
            $footerTwoNav = self::getNavigation(Navigations::NAVTYPE_FOOTER_TWO);
            FatCache::set('footerTwoNav', serialize($footerTwoNav), '.txt');
        }
        return $footerTwoNav;
    }
    ...
    {
        $footerThreeNav = FatCache::get('footerThreeNav', CONF_HOME_PAGE_CACHE_TIME, '.txt');
        if (!empty($footerThreeNav)) {
            ...
            FatCache::set('footerThreeNav', serialize($footerThreeNav), '.txt');
        }
