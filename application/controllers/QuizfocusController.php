...
class QuizfocusController extends MyAppController
{
    ...
    public function getQuestions()
{
    ...
    $examboardId  = isset($posts['examboardid']) ? (int)$posts['examboardid'] : 0;
    $levelId      = isset($posts['levelid']) ? (int)$posts['levelid'] : 0;
    ...
    $conditions = ["subtopic_id = " . $subtopicId];
    if ($examboardId > 0) {
        $conditions[] = "examboard_id = " . $examboardId;
    }
    if ($levelId > 0) {
        $conditions[] = "level_id = " . $levelId;
    }

    $whereSql = implode(" AND ", $conditions);

    $query = "SELECT * FROM tbl_quaestion_bank WHERE $whereSql ORDER BY RAND() LIMIT 15";
