...
if ((($_SERVER['HTTPS'] ?? '') == 'on' || ($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') == 'https') && (defined('CONF_USE_SSL') && CONF_USE_SSL == 0)) {
    // do nothing, allow HTTP
}
if ((($_SERVER['HTTPS'] ?? '') != 'on' && ($_SERVER['HTTP_X_FORWARDED_PROTO'] ?? '') != 'https') && (defined('CONF_USE_SSL') && CONF_USE_SSL == 1)) {
