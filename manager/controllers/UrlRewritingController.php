...
class UrlRewritingController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch = new SearchBase(SeoUrl::DB_TBL, 'seourl');
        if (!empty($post['keyword'])) {
            $condition = $srch->addCondition('seourl_original', 'like', '%' . $post['keyword'] . '%');
            $condition->attachCondition('seourl_custom', 'like', '%' . $post['keyword'] . '%', 'OR');
        }
    ...
    private function getSearchForm(): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Language'), 'seourl_lang_id', Language::getAllNames(), $defaultLangId);
    ...
    private function getForm(): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_HTTP_CODE'), 'seourl_httpcode', SeoUrl::getHttpCodes())->requirements()->setRequired();
