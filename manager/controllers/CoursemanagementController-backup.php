...
public function uploadQuestionBank()
{
  ...
  //  $quotedSubtopicId = $db->quoteVariable($subtopicId);
...
if (!$db->query($sql)) {
   ...
   foreach ($lines as $line) {
    // Pad to at least 12 fields to prevent undefined offset errors
    $line = array_pad($line, 12, '');

    // Destructure line safely
    [
        $question_text,
        $answer_a,
        $answer_b,
    ...
    // Basic validation (you can customize based on required fields)
    ...
    $questionData = [
        ...
        'subcategory'       => '', // Optional or future use
  ...
  //  FatUtility::dieJsonSuccess("✅ {$insertedCount} questions uploaded successfully!");
}
    ...
    public function setup()
    {
        ...
        if ($post['year_id'] == 'add_new') {
            ...
            if (!$db->insertFromArray('course_year', $insertData)) {
                FatUtility::dieJsonError('❌ Failed to insert new year: ' . $db->getError());
            }
 ...
 // Step 1: Fetch course materials
        ...
        //echo '<pre>';print_r( $courseMaterials);die;
        // Step 2: Loop through each course material and fetch questions
        $finalData = [];

        foreach ($courseMaterials as $material) {
            $subtopicId = $material['subtopic']; // Assuming this ID is used as subtopic_id in tbl_question_bank
