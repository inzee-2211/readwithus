...
class ContentPagesController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        if (!empty($post['keyword'])) {
            $cond = $srch->addCondition('p.cpage_identifier', 'like', '%' . $post['keyword'] . '%');
            $cond->attachCondition('p_l.cpage_title', 'like', '%' . $post['keyword'] . '%');
        }
     ...
     */
    public function langForm($pageId = 0, $lang_id = 0, $cpage_layout = 0)
    {
        $this->objPrivilege->canEditContentPages();
        $pageId = FatUtility::int($pageId);
        ...
        $blockLangFrm = $this->getLangForm($pageId, $lang_id, $cpage_layout);
    ...
    public function deleteRecord()
    {
        ...
        $contentPage = new ContentPage($pageId);
        if (!$contentPage->loadFromDb()) {
            FatUtility::dieJsonError(Label::getLabel('LBL_INVALID_REQUEST'));
        }
        if (!$contentPage->canRecordMarkDelete($pageId)) {
            FatUtility::dieJsonError(Label::getLabel('LBL_INVALID_REQUEST'));
        }
        $contentPage->assignValues([ContentPage::tblFld('deleted') => 1]);
        if (!$contentPage->save()) {
            FatUtility::dieJsonError($contentPage->getError());
        }
    ...
    private function getForm(int $pageId = 0): Form
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_Page_Identifier'), 'cpage_identifier');
     ...
     * @return Form
     */
    private function getLangForm(int $pageId = 0, int $lang_id = 0, int $cpage_layout = 0): Form
    {
        $frm = new Form('frmBlockLang');
        $frm->addHiddenField('', 'cpage_id', $pageId);
        $frm->addHiddenField('', 'lang_id', $lang_id);
        $frm->addHiddenField('', 'cpage_layout', $cpage_layout);
        $frm->addRequiredField(Label::getLabel('LBL_Page_Title'), 'cpage_title');
        if ($cpage_layout == ContentPage::CONTENT_PAGE_LAYOUT1_TYPE) {
            $frm->addButton(Label::getLabel('LBL_Backgroud_Image'), 'cpage_bg_image', Label::getLabel('LBL_Upload_Image'), [
                'class' => 'bgImageFile-Js',
                'id' => 'cpage_bg_image',
                'data-file_type' => Afile::TYPE_CPAGE_BACKGROUND_IMAGE,
                'data-frm' => 'frmBlock'
            ]);
            $frm->addTextBox(Label::getLabel('LBL_Background_Image_Title'), 'cpage_image_title');
            $frm->addTextarea(Label::getLabel('LBL_Background_Image_Description'), 'cpage_image_content');
            for ($i = 1; $i <= ContentPage::CONTENT_PAGE_LAYOUT1_BLOCK_COUNT; $i++) {
                $frm->addHtmlEditor(Label::getLabel('LBL_Content_Block_' . $i), 'cpblock_content_block_' . $i);
            }
        } else {
            $frm->addHtmlEditor(Label::getLabel('LBL_Page_Content'), 'cpage_content');
        }
