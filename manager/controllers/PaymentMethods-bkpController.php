...
class PaymentMethodsController extends AdminBaseController
{
    ...
    private function getTxnfeeForm()
    {
        ...
        $fld = $frm->addSelectBox(Label::getLabel('LBL_FEE_TYPE'), 'type', AppConstant::getPercentageFlatArr());
        $fld->requirements()->setIntPositive();
        $fld->requirements()->setRequired();
        $fld = $frm->addTextBox(Label::getLabel('LBL_TXN_FEE'), 'fee');
        $fld->requirements()->setFloatPositive();
        $fld->requirements()->setRange(0, 99);
        $fld->requirements()->setRequired();
