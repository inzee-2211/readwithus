...
class ClassLanguagesController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        $srch->addMultipleFields([
            'IFNULL(tlanglang.tlang_name, tlang.tlang_identifier) as language', 'grpcls_tlang_id as tlang_id', 'COUNT(grpcls_tlang_id) AS totalsold',
        ...
        if (!empty($post['grpcls_tlang_id'])) {
            $srch->addCondition('grpcls_tlang_id', '=', $post['grpcls_tlang_id']);
        } elseif (!empty($post['grpcls_tlang'])) {
            $srch->addCondition('tlanglang.tlang_name', 'LIKE', '%' . trim($post['grpcls_tlang']) . '%');
        }
    ...
    {
        $frm = new Form('frmClassLanguages');
