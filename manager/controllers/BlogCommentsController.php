...
class BlogCommentsController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        if (!empty($post['keyword'])) {
            $cond = $srch->addCondition('bpcomment_author_name', 'like', '%' . $post['keyword'] . '%');
            $cond->attachCondition('bpcomment_author_email', 'like', '%' . $post['keyword'] . '%');
        }
        ...
        $srch->addMultipleFields(['bpcomment_id', 'bpcomment_author_name', 'bpcomment_author_email','bpcomment_content',
            'bpcomment_approved', 'bpcomment_added_on', 'post_id', 'ifnull(post_title, post_identifier) post_title']);
    ...
    public function updateStatus()
    {
        ...
        FatUtility::dieJsonSuccess(Label::getLabel('MSG_BLOG_POST_SETUP_SUCCESSFUL'));
    }
    ...
    private function getForm(int $commentId = 0): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Comment_Status'), 'bpcomment_approved', BlogPost::getCommentStatuses(), '', ['class' => 'small'], '');
    ...
    private function getSearchForm(): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Comment_Status'), 'bpcomment_approved', BlogPost::getCommentStatuses(), '', ['class' => 'small'], 'Select');
