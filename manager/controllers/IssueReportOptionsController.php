...
class IssueReportOptionsController extends AdminBaseController
{
    ...
    public function search()
    {
        ...
        if (!empty($post['keyword'])) {
            $cond = $srch->addCondition('tissueopt_identifier', 'like', '%' . $post['keyword'] . '%');
            $cond->attachCondition('tissueoptlang_title', 'like', '%' . $post['keyword'] . '%');
        }
        $this->sets([
    ...
    private function getForm(): Form
    {
        ...
        $frm->addRequiredField(Label::getLabel('LBL_OPTION_IDENTIFIER'), 'tissueopt_identifier');
