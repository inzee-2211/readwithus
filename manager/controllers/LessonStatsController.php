...
class LessonStatsController extends AdminBaseController
{
    ...
    {
        $frm = new Form('frmLessonStatus');
        $frm->addTextBox(Label::getLabel('LBL_USER'), 'user', '', ['id' => 'user', 'autocomplete' => 'off']);
        $frm->addDateField(Label::getLabel('LBL_DATE_FROM'), 'fromDate', '', ['readonly' => 'readonly']);
        $frm->addDateField(Label::getLabel('LBL_DATE_TO'), 'toDate', '', ['readonly' => 'readonly']);
        ...
        $frm->addButton("", "btn_clear", Label::getLabel('LBL_CLEAR_SEARCH'));
        return $frm;
    }
    ...
    {
        $srch = new SearchBase(SessionLog::DB_TBL, 'sesslog');
