        ...
        switch ($key) {
            ...
            case 'action':
                $ul = $td->appendElement("ul", ["class" => "actions"]);
                    ...
                    $li->appendElement('a', ['href' => 'javascript:void(0)', 'title' =>  Label::getLabel('LBL_EDIT'), 'onclick' => 'form(' . $row['ratrev_id'] . ');'], '<i class="ion-edit icon"></i>', true);
                break;
            case 'ratrev_created':
...
echo $tbl->getHtml();
echo FatUtility::createHiddenFormFromData($postedData, ['name' => 'frmRatingReviewSearchPaging']);
