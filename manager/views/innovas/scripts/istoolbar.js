...
function ISToolbarManager(id) {
    ...
    this.render = function () {
        if (this.tabCtl)
            return this.tabCtl.render();

        var ret = "", flow = false;
        for (var i in this.groups) {
            if (this.groups[i].render)
                ret += this.groups[i].render();
            if (this.groups[i].groupFlow)
                flow = true;
        }

        if (this.groups[i].draggable) {
            if (ret != "")
                return "<div class=\"istoolbar_handler\"><div style=\"cursor:auto\">" + ret + "</div></div>";
        } else {
            if (ret != "")
                return "<div class=\"istoolbar_container\" style=\"" + (flow ? "position:relative;" : "") + "padding:3px 5px 3px 2px;background-image:url(" + this.iconPath + "grpbg.gif)\">" + ret + "</div>";
        }

        for (var i in this.tbars) {
            if (this.tbars[i].render)
                ret += this.tbars[i].render();
        }
        if (ret != "")
            return ret;
        return "";
...
function ISButton(id, icon, text, width, height) {
    ...
    this.setState = function (s) {
        ...
        var btn = this.domObj;
        if (!btn) {
            btn = document.getElementById(this.id).childNodes[0];
            this.domObj = btn;
        }
    ...
    this.toHTML = function () {
        ...
        if (this.icon.indexOf("/") == -1)
            sIconURL = this.container.iconPath + this.icon;
        else
            sIconURL = this.icon;
...
function $mvmsDown(e, el, tbId) {
    ...
    d.onmousemove = function (e) {
        $tbStartDrag_1((isIE ? event : e), tb, document.getElementById(tbId));
    }
    d.onmouseup = function (e) {
        $tbEndDrag((isIE ? event : e), tb)
    }
    ...
    if (d) {
        d.onmousemove = function (e) {
            $tbStartDrag_2((isIE ? d.parentWindow.event : e), tb, document.getElementById(tbId));
        }
        d.onmouseup = function (e) {
            $tbEndDrag((isIE ? d.parentWindow.event : e), tb)
        }
        d.onselectstart = function () {
            return false;
        }
    ...
    if (btn.type == "STD") {
        ...
        if (btn.group != null && btn.group != "") {
            ...
            for (var it in tbar.btns) {
                ...
                if (!tBtn.id)
                    continue;
                if (tBtn.group == btn.group && tBtn.id != btn.id) {
                    tBtn.setState(1);
                    tBtn.checked = false;
                }
            }
...
function showDD(tbId, btnId, ddId) {
    ...
    if (!isDDs[ddId].container)
        isDDs[ddId].container = isTbars[tbId].btns[btnId];
}
;
...
ISTabCtl.prototype.render = function () {
    ...
    if (!isIE)
        s[j++] = "<div style='position:relative'>";
    for (var it in this.tabs) {
        o = this.tabs[it].obj;
        if (!o)
            continue;
...
function ISWindow(id) {
    ...
    this.show = function (opt) {
        ...
        var fn =
                function () {
                    ...
                    if (typeof (me.rt.frm.contentWindow.bodyOnLoad) != "undefined")
                        me.rt.frm.contentWindow.bodyOnLoad();
                };
    ...
    this.close = function () {
        ...
        if (d) {
            if (this.rt.frm.contentWindow.bodyOnUnload)
                this.rt.frm.contentWindow.bodyOnUnload();
    ...
    ISWindow._ddMouseDown = function (ev, elId) {
        ...
        d.onmousemove = function (e) {
            ISWindow._startDrag(e ? e : ev);
        }
        d.onmouseup = function (e) {
            ISWindow._endDrag(e ? e : ev);
        }
        d.onselectstart = function () {
            return false;
        }
