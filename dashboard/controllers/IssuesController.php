...
class IssuesController extends DashboardController
{
    ...
    public function form()
    {
        ...
        if (!$issue->validateRecord($recordId, $recordType)) {
            FatUtility::dieJsonError($issue->getError());
        }
        $frm = $this->getForm();
    ...
    private function getForm(): Form
    {
        ...
        $frm->addSelectBox(Label::getLabel('LBL_Subject'), 'repiss_title', $options)->requirements()->setRequired();
    ...
    public function resolve()
    {
        ...
        $frm = $this->getResolveForm($issue['repiss_record_type']);
     ...
     * @return Form
     */
    private function getResolveForm(int $type): Form
    {
        ...
        if (AppConstant::GCLASS === $type) {
            unset($options[Issue::ACTION_RESET_AND_UNSCHEDULED]);
        }
        $frm->addSelectBox(Label::getLabel('LBL_TAKE_ACTION'), 'reislo_action', $options)->requirements()->setRequired(true);
