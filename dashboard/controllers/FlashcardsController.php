...
class FlashcardsController extends DashboardController
{
    ...
    public function search()
    {
        ...
        $srch->joinTable(TeachLanguage::DB_TBL, 'INNER JOIN', 'tlang.tlang_id = flashcard.flashcard_tlang_id', 'tlang');
        ...
        if (!empty($post['keyword'])) {
            $cond = $srch->addCondition('flashcard_title', 'LIKE', '%' . $post['keyword'] . '%');
            $cond->attachCondition('flashcard_detail', 'LIKE', '%' . $post['keyword'] . '%');
        }
        ...
        $cards = FatApp::getDb()->fetchAll($srch->getResultSet());
    ...
    public function form()
    {
        ...
        if ($flashcardId > 0) {
            ...
            if ($row['flashcard_user_id'] != $this->siteUserId) {
    ...
    public function setup($type = Flashcard::TYPE_OTHER)
    {
        ...
        if (!$card->save()) {
            FatUtility::dieJsonError($card->getError());
        }
