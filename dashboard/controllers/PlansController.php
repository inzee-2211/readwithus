...
class PlansController extends DashboardController
{
    ...
    public function search()
    {
        ...
        if (!empty($post['keyword'])) {
            $cond = $srch->addCondition('plan_title', 'LIKE', '%' . $post['keyword'] . '%');
            $cond->attachCondition('plan_detail', 'LIKE', '%' . $post['keyword'] . '%');
        }
    ...
    public function setup()
    {
        ...
        for ($i = 0; $i < count($_FILES['plan_file']['name']); $i++) {
            if (!empty($_FILES['plan_file']['tmp_name'][$i])) {
                $fileData = [
     ...
     * @param int $planId
     */
    public function viewAssignedPlan($planId)
    {
        $srch = new SearchBase(Plan::DB_TBL);
        $srch->addMultipleFields(['plan_id', 'plan_title', 'plan_level', 'plan_teacher_id', 'plan_detail']);
        $srch->addCondition('plan_id', '=', FatUtility::int($planId));
        ...
        $this->set('data', $row);
        $file = new Afile(Afile::TYPE_LESSON_PLAN_FILE);
        $this->set('planFiles', $file->getFiles($planId));
        $this->_template->render(false, false);
    }
