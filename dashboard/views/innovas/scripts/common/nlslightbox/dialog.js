...
function modalDialog(url, width, height, title) {
    ...
    var clean = function () {
        if (oUtil)
            oUtil.onSelectionChanged = null;
    };

    if (title)
        box.open({url: url, type: 'iframe', width: width + 'px', height: height + 'px', title: title, adjY: '20px', overlay: false, parent: document.body, onClose: function () {
                clean();
            }});
    else
        box.open({url: url, type: 'iframe', width: width + 'px', height: height + 'px', adjY: '20px', overlay: false, parent: document.body, onClose: function () {
                clean();
            }});
