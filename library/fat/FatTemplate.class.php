...
class FatTemplate
{
        ...
        if ($includeCommon && file_exists($pth)) {
            ...
            foreach ($arrCommonfiles as $fl) {
                ...
                if ($mergeFiles) {
                    ...
                    $str .= '<link rel="stylesheet" type="text/css" href="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'cssCommon', [], $use_root_url, false) . '&f=' . rawurlencode($fl) . '&min=0&sid=' . $time, CONF_DEF_CACHE_TIME, '.css') . '" />' . "\n";
                }
            }
            if ($mergeFiles) {
                $str .= '<link rel="stylesheet" type="text/css" href="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'cssCommon', [], $use_root_url, false) . '&min=1&sid=' . $last_updated, CONF_DEF_CACHE_TIME, '.css') . '" />' . "\n";
            }
        }
        ...
        if (count($this->arr_page_css) > 0) {
            ...
            foreach ($this->arr_page_css as $val) {
                ...
                if ($mergeFiles) {
                    ...
                    $str .= '<link rel="stylesheet" type="text/css" href="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'css', [], $use_root_url, false) . '&f=' . rawurlencode($val) . '&min=0&sid=' . $time, CONF_DEF_CACHE_TIME, '.css') . '" />' . "\n";
                    $str .= str_replace('/public', '', $str);
                }
            }
            if ($mergeFiles) {
                $str .= '<link rel="stylesheet" type="text/css" href="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'css', [], $use_root_url, false) . '&f=' . rawurlencode(implode(',', $this->arr_page_css)) . '&min=1&sid=' . $last_updated, CONF_DEF_CACHE_TIME, '.css') . '" />' . "\n";
            }
        }
        ...
        if ($includeCommon) {
            ...
            foreach ($arrCommonfiles as $fl) {
                ...
                if ($mergeFiles) {
                    ...
                    $str .= '<script src="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'jsCommon', [], $use_root_url, false) . '&f=' . rawurlencode($fl) . '&min=0&sid=' . $time, CONF_DEF_CACHE_TIME, '.js') . '"></script>' . "\n";
                }
            }
            if ($mergeFiles) {
                $str .= '<script src="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'jsCommon', [], $use_root_url, false) . '&min=0&sid=' . $last_updated, CONF_DEF_CACHE_TIME, '.js') . '"></script>' . "\n";
            }
        }
        if (count($this->arr_page_js) > 0) {
            ...
            foreach ($this->arr_page_js as $val) {
                ...
                if ($mergeFiles) {
                    ...
                    $str .= '<script src="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'js', [], $use_root_url, false) . '&f=' . rawurlencode($val) . '&min=0&sid=' . $time, CONF_DEF_CACHE_TIME, '.js') . '" ></script>' . "\n";
                }
            }
            if ($mergeFiles) {
                $str .= '<script src="' . FatCache::getCachedUrl(FatUtility::generateUrl('JsCss', 'js', [], $use_root_url, false) . '&f=' . rawurlencode(implode(',', $this->arr_page_js)) . '&min=1&sid=' . $last_updated, CONF_DEF_CACHE_TIME, '.js') . '" ></script>' . "\n";
            }
        }
        /* Include JS Ends */
    ...
    public function includeTemplate($tplPath, $variables = [], $convertVariablesToHtmlentities = true)
    {
