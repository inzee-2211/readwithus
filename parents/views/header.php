...
/**
 * Parent portal safety defaults
 * We must NOT assume learner/teacher session exists.
 */
$siteUser     = $siteUser ?? [];
$siteUserId   = isset($siteUserId) ? (int) $siteUserId : 0;
$siteUserType = isset($siteUserType) ? (int) $siteUserType : 3; // 3 = Parent (our convention)

/* Safe user meta for header/profile */
$userFirstName = $siteUser['user_first_name'] ?? Label::getLabel('LBL_PARENT');
$userLastName  = $siteUser['user_last_name']  ?? '';
$userTimezone  = $siteUser['user_timezone']   ?? FatApp::getConfig(
    'CONF_SITE_TIMEZONE',
    FatUtility::VAR_STRING,
    'UTC'
);
$hasUserTimezone = !empty($userTimezone);

/* Body profile type (for CSS class) */
$bodyProfileType = 'parent';
if ($siteUserType == User::TEACHER) {
    $bodyProfileType = 'teacher';
} elseif ($siteUserType == User::LEARNER) {
    $bodyProfileType = 'learner';
}
?>
<!doctype html>
...
<head>
    ...
    $mainDashboardClass = (($controllerName == 'Teacher' || $controllerName == 'Learner') && $actionName == "index")
        ? "main-dashboard"
        : '';
    ?>
...
<body
    class="dashboard-<?php echo $bodyProfileType . ' ' . strtolower($controllerName) . ' ' . strtolower($actionName) . ' ' . $mainDashboardClass . ' ' . $isPreviewOn; ?>">
<?php
                        ...
                        <li class="menu__item menu__item-languages">
                            ...
                            <div id="languages-nav" class="menu__dropdown">
                                <div class="menu__dropdown-head">
                                    <span class="uppercase small bold-600">
                                        <?php echo Label::getLabel('LBL_CHANGE_LANGUAGES'); ?>
                                    </span>
                                </div>
                                            ...
                                            <?php foreach ($siteLanguages as $language) { ?>
                                                <li
                                                    ...
                                                    <a href="javascript:void(0)"
                                                       <?php echo ($siteLangId != $language['language_id']) ? 'onclick="setSiteLanguage(' . $language['language_id'] . ')"' : ''; ?>>
                                        ...
                                        <ul>
                                            <?php foreach ($siteCurrencies as $currency) { ?>
                                                <li
                                                    ...
                                                    <a
                                                        <?php echo ($siteCurrency['currency_id'] != $currency['currency_id']) ? 'onclick="setSiteCurrency(' . $currency['currency_id'] . ')"' : ''; ?>
            ...
            <div class="sidebar__head">
                <figure class="logo">
                    <a href="<?php echo MyUtility::makeUrl('', '', [], CONF_WEBROOT_FRONT_URL); ?>">
                        ...
                        <?php } else { ?>
                            <img src="<?php echo FatCache::getCachedUrl(MyUtility::makeFullUrl('Image', 'show', [Afile::TYPE_FRONT_LOGO, 0, Afile::SIZE_LARGE], CONF_WEBROOT_FRONT_URL), CONF_DEF_CACHE_TIME, '.jpg'); ?>"
                        ...
                        <a href="#profile-target" class="trigger-js profile__trigger">
                            <div class="profile__meta d-flex align-items-center">
                                <div class="profile__media margin-right-4">
                                    <div class="avtar"
                                         data-title="<?php echo CommonHelper::getFirstChar($userFirstName); ?>">
                                        <?php
                                        if ($siteUserId > 0) {
                                            echo '<img src="' .
                                                FatCache::getCachedUrl(
                                                    MyUtility::makeUrl(
                                                        'Image',
                                                        'show',
                                                        [Afile::TYPE_USER_PROFILE_IMAGE, $siteUserId, Afile::SIZE_SMALL],
                                                        CONF_WEBROOT_FRONT_URL
                                                    ),
                                                    CONF_DEF_CACHE_TIME,
                                                    '.jpg'
                                                ) . '?t=' . time() . '" alt="' . $userFirstName . '" />';
                                        }
                                        ?>
                                    </div>
                                </div>
                                <div class="profile__details">
                                    <h6 class="profile__title">
                                        <?php echo trim($userFirstName . ' ' . $userLastName); ?>
                                    </h6>
                                    <?php
                                    if ($siteUserType == User::TEACHER) {
                                        $loggedAs = 'LBL_LOGGED_IN_AS_A_TEACHER';
                                    } elseif ($siteUserType == User::LEARNER) {
                                        $loggedAs = 'LBL_LOGGED_IN_AS_A_LEARNER';
                                    } else {
                                        $loggedAs = 'LBL_PARENT_PORTAL';
                                    }
                                    ?>
                        ...
                        <div id="profile-target" class="profile__target">
                            <div class="profile__target-details">
                                <table>
                                    ...
                                    <?php } ?>
                                    <?php if ($hasUserTimezone) { ?>
                                        <tr>
                                            <th><?php echo label::getLabel('LBL_TIME_ZONE'); ?></th>
                                            <td>
                                                <?php
                                                echo MyDate::formatDate(date('Y-m-d H:i:s'), 'H:i', $userTimezone);
                                                try {
                                                    $tzObj  = new DateTimeZone($userTimezone);
                                                    $offset = (new DateTime("now", $tzObj))->format('P');
                                                    echo ' (' . Label::getLabel('LBL_UTC') . " " . $offset . ')';
                                                } catch (Exception $e) {
                                                    // silently ignore bad timezone
                                                }
                                                ?>
                                            </td>
                                        </tr>
                                    ...
                                    <?php if ($siteUserType == User::TEACHER) { ?>
                                        <?php if (!empty($profileProgress['isProfileCompleted'])) { ?>
                                            <a href="<?php echo MyUtility::makeFullUrl('teachers', 'view', [$siteUser['user_username']], CONF_WEBROOT_FRONTEND); ?>"
                                               class="btn btn--bordered color-third btn--block margin-top-2">
                                                <?php echo label::getLabel('LBL_View_Public_Profile'); ?>
                                            </a>
                                        <?php } ?>
            ...
            <div class="sidebar__body">
                <div class="sidebar__scroll">
                    <div id="primary-nav" class="menu-offset">
                        <?php
                        // Parent sidebar (dashboard/children/subscriptions/wallet)
                        $templateVariable = [
